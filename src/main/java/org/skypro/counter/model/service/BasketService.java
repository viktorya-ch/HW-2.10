...
public class BasketService {
    ...
    public UserBasket getUserBasket(){
        return new
        UserBasket (productBasket.getProducts().entrySet().stream().map(entry ->{UUID productId=entry.getKey();
...
return new BasketItem(product,quantity);}).collect(Collectors.toList()));
    }
    }
